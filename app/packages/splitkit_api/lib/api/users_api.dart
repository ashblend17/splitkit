...
class UsersApi {
  ...
  Future<Response> updateMeWithHttpInfo(UserUpdate userUpdate,) async {
    // ignore: prefer_const_declarations
    final path = r'/api/v1/me';
  ...
  ///
  /// * [UserUpdate] userUpdate (required):
  Future<UserOut?> updateMe(UserUpdate userUpdate,) async {
    final response = await updateMeWithHttpInfo(userUpdate,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
